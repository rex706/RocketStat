...
//Memory scan for known patterns with with dynamic and/or unknown bytes.

// sigScan C# Implementation - Written by atom0s [aka Wiccaan]
//                           - Edited by rex706  [aka Duke Webelos]
// Class Version: 2.0.1
//
// [ CHANGE LOG ] -------------------------------------------------------------------------
//
...
//          - Better usage examples and explanation.
...
// [ USAGE ] ------------------------------------------------------------------------------
//
// FindPattern takes in the pattern byte array (bytes to be searched), a
// string byte mask (x indicates known byte, ? is unknown and ignored), and the
// desired return index int (the byte address in the array you want).
//
// Examples:
//
// ---[ KNOWN MEMORY REGION ]---
...
//      //Alternative:
//
//      SigScan _sigScan = new SigScan(someProccess, new IntPtr(0x123456), 0x1000);
//      IntPtr pAddr = _sigScan.FindPattern(new byte[]{ 0xFF, 0xFF, 0xFF, 0xFF, 0x51, 0x55, 0xFC, 0x11 }, "xxxx?xx?", 12);
//
// ---[ ENTIRE PROCESS MEMORY ]---
//
//      ProcessModuleCollection myProcessModuleCollection = someProcess.Modules;
//
//      SigScan _sigScan = new SigScan();
...
//
//          pAddr = _sigScan.FindPattern(new byte[]{ 0xFF, 0xFF, 0xFF, 0xFF, 0x51, 0x55, 0xFC, 0x11 }, "xxxx?xx?", 0);
//
